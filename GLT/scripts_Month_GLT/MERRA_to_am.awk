...
BEGIN {
    ...
    T_col = 4       # column number for temperature [K]
    x_H2O_col = 9   # column number for H2O vmr in ppmv
    x_O3_col = 14   # column number for O3 vmr in ppmv
...

{
    if (n > 0 && P[n-1] > P_ground && $P_col < P_ground) {
        ...
        x_H2O[n] = r * x_H2O[n-1] + (1.0 - r) * $x_H2O_col
